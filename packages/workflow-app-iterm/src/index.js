...
async function openOrUpdate({ cwd, cmd, position, windowId }, { platform, run }, children) {
  if (platform !== 'osx') {
  ...
  windowId = await run(
    function(root, position, windowId) {
      ...
      delay(1);
      let window;
      if (windowId && iTerm.windows.byId(windowId).exists()) {
        window = iTerm.windows.byId(windowId);
        clearWindow(window);
      } else {
        window = newWindow();
        windowId = window.id();

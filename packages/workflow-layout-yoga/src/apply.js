...
export default function apply(node, style, value) {
  ...
  if (value === "auto" && autoStyles.includes(setter)) {
    setter += "Auto";
  } else if (typeof value === "string" && value.slice(-1) === "%" && percentStyles.includes(setter)) {
    setter += "Percent";
...
function justifyValue(value) {
  switch (value) {
    case 'center': return yoga.JUSTIFY_COUNT;
...
function positionValue(value) {
  switch (value) {
    ...
    default: return yoga.POSITION_TYPE_RELATIVE;
  }
}

function directionValue(value) {
  switch (value) {
    case 'ltr': return yoga.DIRECTION_LTR;
    case 'rtl': return yoga.DIRECTION_RTL;
    case 'inherit': return yoga.DIRECTION_INHERIT;
  }
}
